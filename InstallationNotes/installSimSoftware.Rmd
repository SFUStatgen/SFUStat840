...
### 2.3. Installing `msprime` and `tskit` into a Python "environment".
...
### 2.4. Installing `pyslim`

Unfortunately `pyslim` is not available on the
`conda-forge` website that hosts conda-installable packages. However, I have built and 
uploaded our own conda-installable version of `pyslim`
to the class GitHub site and written a function 
`conda_install_local()` to simplify its installation 
from within R. Both the package and R 
...
account on a Mac Pro housed in Brad McNeney's office.
You will have to make an appointment with Brad to log in from the console the first time you
use it. After the initial login from the console in his office, you will be able to log in remotely. You will
